'''
@author: avalanchy (at) google mail dot com, modifications by calendis1 (at) gmail dot com
@version: 0.1; python 3.6; Kubuntu 16.10 Yaketty Yak
@date: 2012-04-08, 2017-03-10
...
'''
Modified for use in the game Moderately Seafaring by Bert Myroon
'''
...
class RefreshingMenu(Menu):
    """docstring for RefreshingMenu"""
    def __init__(self):
        super(RefreshingMenu, self).__init__()
        self.move_values = (0,0)

    def refresh(self):
        self.set_fontsize(self.font_size)
        self.move_menu(-self.move_values[0], -self.move_values[1])
        
        if len(self.init_arguments) == 3:
            self.init([self.init_arguments[0], self.init_arguments[1], self.init_arguments[2]], screen)
        elif len(self.init_arguments) == 4:
            self.init([self.init_arguments[0], self.init_arguments[1], self.init_arguments[2], self.init_arguments[3]], screen)

BACKGROUND_COLOUR = (74,104,200)

class MainMenu(RefreshingMenu):
    """docstring for MainMenu"""
    def __init__(self):
        super(MainMenu, self).__init__()
        self.set_colors((0,255,0),(BACKGROUND_COLOUR[0]-10,BACKGROUND_COLOUR[1]-10,BACKGROUND_COLOUR[2]-10), BACKGROUND_COLOUR)
        self.font_size = 30
        self.init_arguments = ["New Game", "Continue", "Options", "Quit"]

        self.init(self.init_arguments, screen)
        

class StartMenu(RefreshingMenu):
    """docstring for StartMenu"""
    def __init__(self):
        super(StartMenu, self).__init__()
        self.move_values = (377, 270)
        self.font_size = 20
        self.init_arguments = ["Party", "Items", "Save", "Pause"]
        self.move_menu(-self.move_values[0],-self.move_values[1])
        self.set_fontsize(20)
        self.init(["Party", "Items", "Save", "Pause"], screen)

class SaveMenu(RefreshingMenu):
    """docstring for SaveMenu"""
    def __init__(self):
        super(SaveMenu, self).__init__()
        self.font_size = 40
        self.init_arguments = ["Save File 1", "Save File 2", "Save File 3"]
        self.set_fontsize(40)
        self.init(self.init_arguments, screen)

class LoadMenu(RefreshingMenu):
    """docstring for LoadMenu"""
    def __init__(self):
        super(LoadMenu, self).__init__()
        self.set_colors((0,255,0),(BACKGROUND_COLOUR[0]-10,BACKGROUND_COLOUR[1]-10,BACKGROUND_COLOUR[2]-10), BACKGROUND_COLOUR)
        self.font_size = 40
        self.init_arguments = ["Load File 1", "Load File 2", "Load File 3"]
        self.set_fontsize(40)
        self.init(self.init_arguments, screen)

class ItemMenu(RefreshingMenu):
    ...
    def __init__(self, items):
        ...
        self.init_arguments = []
        for item in items:
            self.init_arguments.append(item.name)
        self.init(self.init_arguments, screen)

class PartyMenu(RefreshingMenu):
    ...
    def __init__(self, Party):
        ...
        for party_member in party:
            self.init_arguments.append(party_member)
        self.init(self.init_arguments, screen)
